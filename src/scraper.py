...
def get_price(origin, destination):
    ...
    print(f"API returned no usable price for {origin} -> {destination}", flush=True)
    print(f"API raw data: {data}", flush=True)
...
def run():
    ...
    for route in routes:
        ...
        print(f"Price found for {origin} -> {destination}: {current_price} USD", flush=True)
        ...
        if price_dropped:
            ...
            if not alert_already_sent(route_id, current_price):
                send_alert(origin, destination, current_price, last_price)
                save_alert(route_id, current_price)
            else:
                print(
                    f"Alert already sent for route {route_id} at price {current_price}",
                    flush=True
                )
